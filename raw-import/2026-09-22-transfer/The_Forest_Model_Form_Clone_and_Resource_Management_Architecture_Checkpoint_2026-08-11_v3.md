...
# Forest Lineage Vocabulary — Ortet, Ramet, Genet, and Colony

## Design decision

The Forest should present its own botanical terminology **first** in user-facing interfaces, while ordinary technical terms remain available underneath through tooltips, help menus, documentation, developer notes, and code.

The intended vocabulary is:

```text
Tree
= enduring AI identity

Ortet
= original execution context in that Tree's lineage

Ramet
= cloned execution context belonging to the same Tree

Genet / Colony
= two contextual Forest terms for essentially the same
  collection of related execution contexts belonging to one Tree
```

### Primary user-facing language

Forest interfaces may prefer:
...
rather than leading with generic terms such as:
...
Technical translations should remain easy to discover.

Example tooltip:

```text
Ramet
A cloned execution context belonging to the same Tree.
Technical equivalent: Clone execution context.
```
...
```text
Genet
Also called a Colony in some Forest views.
The Ortet and all Ramets belonging to one Tree.
```

## Genet and Colony are contextual synonyms

`Genet` and `Colony` should **not** become two separate state objects.

They describe essentially the same underlying grouping from different presentation perspectives.

Useful contextual distinction:

```text
"Maple Genet"
→ emphasizes lineage, origin, and relationship

"Maple Colony"
→ emphasizes the collection of operating members
```
...
Status:
"3 Ramets active in the Maple Colony"

Lineage:
"This Ramet belongs to the Maple Genet."
```

Humans can understand these as contextual near-synonyms, similar to ordinary English using different words for nearly the same concept in different settings.

## One canonical technical representation underneath

The code must not create duplicate concepts such as:
...
when both would identify the same Tree grouping.

Prefer one canonical technical representation, such as:
...
```yaml
tree_id: maple
execution_context_id: photo-7f3a
lineage_role: ramet
is_main: false
```

Presentation layers may choose whether to display the shared group as `Genet` or `Colony`.

## Lineage role is not the same as operational role

`Ortet` and `Ramet` describe lineage.

`Main` describes an operational role.

Therefore Forest must **not** permanently encode:

```text
ortet == main
```

A future valid state may be:
...
The Ortet remains the Ortet because it is the original lineage context.
The Ramet may become Main because Main is an operational assignment.

This distinction should remain explicit in state and documentation.
...
Genet / Colony
= the Tree's complete related execution-context group
```

Each execution context remains independently controllable:
...
This does not change the previously frozen execution hierarchy:
...
It adds lineage/presentation semantics to the execution-context layer without changing its technical role.

## New vocabulary invariants

> **Forest terminology is primary in the user experience; technical terminology remains available underneath.**

> **Ortet means original lineage context, not permanently "the Main."**

> **Ramet means cloned execution context.**

> **Genet and Colony are contextual synonyms for the same underlying Tree grouping.**

> **Genet and Colony must not create duplicate authoritative state.**

> **Main is an operational role and may move between execution contexts.**

> **Code should use stable technical identifiers even when the UI uses botanical flavor language.**

---

# Chat-Bar Control Surface — Bones Only

## Status

Design direction captured for future implementation. Do not implement the full visual UI during Phase 14.11. For now, preserve only the semantic/control architecture needed so the later UI can attach cleanly.

The current visual draft is a strong concept sketch, not a final layout. Main refinement: controls should sit much closer to the actual chat/composer bar so users read them as part of message composition rather than as a separate dashboard. Exact spacing, styling, animation, iconography, and visual polish remain intentionally unfrozen.

## Composer layout direction

Future composer structure:

```text
Prompt Delivery Strip
        ↓
Chat / Composer Bar
        ↓ or beside
Mixer-style Tree Controls
```
...
```text
ABOVE COMPOSER
[ Inject ○ ] [ Queue ● ] [ Interrupt ○ ]

COMPOSER
[ attachment ]  Type your message...  [ mixer button / controls ] [ Send ]

RIGHT / ADJACENT TO COMPOSER
Vertical "volume mixer" control panel
```

Exact placement may change later.

## Prompt Delivery Strip

The delivery selector is horizontal and directly above the chat bar.
...
These are presentation labels only and must remain replaceable.

Each option is a separate button with two interaction targets:

```text
Click/tap the word or main button area
→ choose this delivery mode for the current prompt only

Click/tap the small circle inside the button
→ set that delivery mode as the preferred default
```

The preferred default persists for future prompts. A one-prompt choice is transient. After sending, the selected delivery mode is locked to that submitted prompt and must not be affected by later composer changes.

The circle should visually communicate the saved default. Current visual idea: a small circle gains an outer ring that grows around it when selected as the preferred default and remains visible while that option is saved. Exact animation/style remains unfrozen.

Stable semantic IDs should sit underneath presentation labels:
...
```text
Inject
→ incorporate new user input into active work at the next safe orchestration/runtime checkpoint

Queue
→ allow current work to finish, then process the new prompt

Interrupt
→ stop/cancel current work at a safe boundary and prioritize the new prompt
```

The UI must not imply arbitrary mid-token model mutation where a runtime cannot support it.

## Mixer-style Tree Controls

The future option panel should resemble a compact vertical audio/volume mixer. All channels move upward.

Reasoning and Model Form are the first channels. Future controls can be added horizontally as additional mixer channels without redesigning the control system.

Conceptual example:

```text
      MODEL FORM      REASONING       FUTURE
          ●               ●              ○
          │               │              │
          │               │              │
          ○               ○              ●
...
Controls should generally be discrete/snap-to-value when the underlying setting is discrete.
...
Model Form
Small / Big
with Auto represented as policy rather than as a fake middle-size model
```

When Model Form is Auto, the visual control may later show both `Auto enabled` and the currently resolved Small/Big state.

## Same control engine, many surfaces

The mixer, keyboard shortcuts, touch controls, controller bindings, help menus, and future UI buttons must all invoke the same semantic actions. There must not be separate keyboard/chat-bar/touch logic paths.
...
Forest terminology, theme, labels, icons, colors, animations, and visual metaphors are presentation-layer concerns. Core state and logic must not depend on labels such as Ortet, Ramet, Genet, Colony, Inject, Queue, Interrupt, Light, Normal, or Deep.

Core implementation should prefer stable technical semantics such as:
...
A complete future theme change should require terminology/theme remapping, not core rewrites.

## Platform and device independence

No Qubes/Linux-specific key binding is a core Forest default. Input bindings must be configurable, with universal cross-platform defaults where practical and optional convenience profiles for Linux, Qubes, Windows, macOS, touch, controllers, and user-specific layouts.

Existing personal numpad/i3/Qubes bindings remain valid as user/platform-specific adapters. Help/tooltips should derive shortcut labels from the active binding registry where practical instead of hardcoding keys.

## Accessibility

Visual-first controls must retain non-visual meaning. Each slider/button needs accessible labels and values through the rendering platform, with tooltips/help exposing technical meaning when the normal UI uses Forest flavor terminology.

## Current implementation rule

Phase 14.11 should build only the bones required by this future UI:

```text
stable semantic IDs
runtime-neutral state
execution-context-local control
clear scope semantics
serializable configuration where appropriate
replaceable terminology/presentation
configurable bindings
```

Do not build the final composer UI, animations, full mixer renderer, final visual theme, full delivery-strip renderer, or platform-specific GUI polish during this phase.
