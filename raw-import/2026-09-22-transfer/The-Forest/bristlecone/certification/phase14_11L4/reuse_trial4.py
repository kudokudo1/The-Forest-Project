...
OUTPUT = (
    ...
    / "benchmarks"
    / "phase14_11L"
    / "l4_same_session_trial4_raw.json"
)
...
IN_PROGRESS = (
    ...
    / "benchmarks"
    / "phase14_11L"
    / "l4_same_session_trial4_in_progress.json"
)
...
if OUTPUT.exists():
    raise RuntimeError(
        "Successful L.4 Trial 4 artifact "
        "already exists."
    )

if IN_PROGRESS.exists():
    raise RuntimeError(
        "L.4 Trial 4 in-progress artifact "
        "already exists."
    )
...
print("=== 14.11L.4 SAME-SESSION REUSE — TRIAL 4 ===")
print()
...
try:
    ...
    failed = OUTPUT.with_name(
        "l4_same_session_trial4_failed_"
        + datetime.now(
            timezone.utc
        ).strftime(
            "%Y%m%dT%H%M%SZ"
