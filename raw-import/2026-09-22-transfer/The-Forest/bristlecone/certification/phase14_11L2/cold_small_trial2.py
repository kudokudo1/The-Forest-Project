...
SUCCESS_PATH = (
    OUTPUT_DIR
    / "l2_cold_small_trial2_raw.json"
)

IN_PROGRESS_PATH = (
    OUTPUT_DIR
    / "l2_cold_small_trial2_in_progress.json"
)
...
if before["resident_models"]:
    raise RuntimeError(
        "Cold precondition failed: "
        "a model is already resident."
    )

if before["llama_server"]:
    raise RuntimeError(
        "Cold precondition failed: "
...
if before["runtime_sockets"]:
    raise RuntimeError(
        "Cold precondition failed: "
        "runtime socket is already active."
    )
