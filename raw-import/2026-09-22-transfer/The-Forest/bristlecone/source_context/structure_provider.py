...
SOURCE_STRUCTURE_CACHE_SCHEMA_VERSION = 1

SOURCE_STRUCTURE_CACHE_NAMESPACE = (
    "source-context"
)

PYTHON_STRUCTURE_CACHE_TYPE = (
    "python-structure"
)


class SourceStructureProviderError(
    ValueError
):
    """Invalid derived source-structure request."""


class PythonSourceStructureProvider:
    """
    Shared provider for immutable Python source
    structures.

    The provider:

    - consumes immutable SourceContentSnapshot;
    - shares derived AST structure through the
      Forest CacheCoordinator;
    - single-flights identical concurrent builds;
    - keeps different exact source versions
    ...
    def __init__(
        ...
        if not isinstance(
            ...
            raise SourceStructureProviderError(
    ...
    def _cached(
        ...
        entry = (
            self._cache_coordinator.get(
                ...
                schema_version=(
                    SOURCE_STRUCTURE_CACHE_SCHEMA_VERSION
                ),
            )
        )
    ...
    def structure_for(
        ...
        with self._condition:
            ...
            # A producer may have completed between
            # our first cache lookup and acquiring
            # the flight lock.
            cached = self._cached(
                key,
                dependencies,
