...
class TurnHistoricalRetrievalBridge:
    ...
    def resolve(
        ...
        # A completely empty captured turn would otherwise
        # bypass both snapshot-safe sticky lookup methods.
        #
        # Perform one empty read solely to validate the
        # retained snapshot against current policy generation.
        # This lookup is read-only and cannot clear or
        # downgrade newer sticky state.
        if (
            not stamp.user_context_record_ids
            and not stamp.operational_learning_record_ids
        ):
            context_state.lookup_sticky_user_context_for_snapshot(
                stamp,
                (),
            )
