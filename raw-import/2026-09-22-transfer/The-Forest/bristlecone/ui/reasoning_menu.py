...
def get_presentation_request(
    ...
    """Load Forest presentation intent without platform policy."""
    ...
    from ui.presentation import (
        reasoning_menu_presentation,
    )

    return reasoning_menu_presentation(
        menu_mode
    )


def get_presentation_adapter(
    request=None,
):
    """Select the best available Forest presentation adapter."""
    ...
    from ui.presentation_adapters import (
        select_presentation_adapter,
    )

    return select_presentation_adapter(
        request
    )


def get_keybinding_registry():
    """Load current user-configurable Forest UI bindings."""
    ...
    from ui.keybindings import (
        ReasoningKeyBindingRegistry,
    )

    return ReasoningKeyBindingRegistry()


def print_keybindings():
    registry = get_keybinding_registry()
    ...
    for action_id, key in (
        ...
        print(
            f"  {action_id}: "
            f"{registry.display_for(action_id)} "
            f"({key})"
        )


def print_presentation_capabilities(
    menu_mode,
):
    """Print adapter negotiation without opening a window."""

    request = get_presentation_request(
        menu_mode
    )

    adapter = get_presentation_adapter(
        request
    )
...
def print_presentation_contract(
    menu_mode,
):
    request = get_presentation_request(
        menu_mode
...
class ReasoningWindow:
    ...
    def __init__(
        ...
        *,
        ...
        self.presentation_adapter.prepare_window(
            self.window,
            self.presentation,
        )
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Print current Forest reasoning "
            "menu keybindings."
        ),
    )
    ...
    if args.presentation_contract:
        print_presentation_contract(
            menu_mode
        )

        return 0

    if args.presentation_capabilities:
        print_presentation_capabilities(
            menu_mode
        )
