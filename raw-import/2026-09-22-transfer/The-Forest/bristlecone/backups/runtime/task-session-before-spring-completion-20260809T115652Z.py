...
class TaskSessionManager:
    ...
    def apply_spring_restoration(
        ...
        def build_application_plan(
            ...
            # Temporary capability restoration has its
            # own lifecycle/permission semantics. Do not
            # silently pretend those resources were
            # restored through the Workshop transaction.
            if restore_temporary:
                raise TaskSessionError(
                    "Controlled Spring restoration "
                    "does not yet apply temporary "
                    "capabilities. Leave them dormant "
                    "until the temporary activation "
                    "path is wired."
                )
        ...
        with self._state_write_lock():
            ...
            try:
                ...
                # Keep the established active-state
                # bookkeeping current.
                if (
                    str(
                        selected_adapter
