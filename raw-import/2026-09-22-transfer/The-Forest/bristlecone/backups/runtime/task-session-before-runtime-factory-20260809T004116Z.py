...
class TaskSessionManager:
    ...
    def get_active_runtime_toolsets(self, state):
        """Read active toolsets through the injected runtime adapter."""
    ...
    def end_task(self, state=None, persist=False):
        ...
        retired_runtime_sessions = copy.deepcopy(
            current["runtime_sessions"]
        )
    ...
    def temporary_turn(
        ...
        transaction = None
        turn_runtime_adapter = self.runtime_adapter

        try:
            if plan["runtime_toolsets"]:
                if turn_runtime_adapter is not None:
                    try:
                        transaction = (
                    ...
                    transaction = (
                        self.begin_temporary_runtime_toolsets(
                            plan,
                            state=state,
                        )
                    )
            ...
            if transaction is not None:
                if turn_runtime_adapter is not None:
                    ...
                    self.restore_temporary_runtime_toolsets(
                        transaction,
                        state=state,
                    )
