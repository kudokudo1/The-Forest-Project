...
class TaskSessionManager:
    ...
    def _reasoning_control_mapping_for_context(
        self,
        state,
        execution_context_id,
    ):
        """Read one execution context's Reasoning mapping.

        This helper intentionally does not fall back to the
        legacy top-level reasoning_control field. Legacy
        migration/default-context behavior is handled at the
        compatibility boundary rather than broadcast to every
        Ortet/Ramet.
        """
        ...
        if (
            ...
            raise TaskSessionError(
                "Reasoning control requires a non-empty "
                "execution_context_id."
            )

        context_id = execution_context_id.strip()
        ...
        if not isinstance(controls, dict):
            raise TaskSessionError(
                "Forest reasoning_controls state "
                "must be a mapping."
            )
    ...
    def _write_reasoning_control_mapping_for_context(
        self,
        state,
        execution_context_id,
        mapping,
    ):
        """Purely write one execution context's Reasoning mapping."""
    ...
    def get_reasoning_control(
        ...
        """Read normalized execution-context Reasoning state."""
        ...
        try:
            ...
            mapping = (
                self._reasoning_control_mapping_for_context(
                    state,
                    context_id,
                )
            )

            # Legacy compatibility belongs only to the
            # historical default execution context.
            if (
                mapping is None
                and context_id == "ctx-default"
            ):
                mapping = state.get(
                    "reasoning_control"
                )
    ...
    def _mutate_reasoning_control(
        ...
        *,
        ...
        """Apply one context-local Reasoning mutation."""
        ...
        def apply(source):
            ...
            # Pre-Colony compatibility is deliberately
            # restricted to ctx-default.
            if (
                current_mapping is None
                and context_id == "ctx-default"
            ):
                current_mapping = source.get(
                    "reasoning_control"
                )
            ...
            if changed:
                working = (
                    self
                    ._write_reasoning_control_mapping_for_context(
                        working,
                        context_id,
                        mapping,
                    )
                ...
                # Non-default Ortets/Ramets never write
                # the legacy Tree-level field.
                if context_id == "ctx-default":
                    working[
                        "reasoning_control"
                    ] = copy.deepcopy(
                        mapping
    ...
    def _finalize_reasoning_control_after_success(
        ...
        *,
        ...
        if not persist:
            ...
            if execution_context_id == "ctx-default":
                updated_working[
                    "reasoning_control"
                ] = copy.deepcopy(
                    updated_mapping
    ...
    def _cleanup_failed_model_form_handoff_transaction(
        self,
        *,
        working,
        frozen_turn,
        ...
        rollback_guard,
        restore_binding,
        transaction_session_ids,
    ):
        """Safely repair/clean one failed Model Form handoff.

        Forest rollback always happens before runtime cleanup.

        If rollback loses a CAS race, current durable Forest
        truth is inspected before cleanup. A session that is
        currently canonical is never retired.

        If current durable truth cannot be inspected safely,
        runtime cleanup fails closed: leaking an obsolete
        session is safer than ending an unknown canonical one.
        """
        ...
        if not isinstance(
            ...
            raise TaskSessionError(
                "Failed handoff cleanup requires "
                "Forest state as a mapping."
            )
        ...
        if not isinstance(
            ...
            raise TaskSessionError(
                "Failed handoff cleanup requires "
                "a session-ID collection."
            )

        candidate_session_ids = []

        for session_id in transaction_session_ids:
            if session_id is None:
                continue
            ...
            if (
                session_id
                and session_id
                not in candidate_session_ids
            ):
                candidate_session_ids.append(
                    session_id
                )
        ...
        if persist:
            if not isinstance(
                ...
                raise TaskSessionError(
                    "Persistent failed handoff cleanup "
                    "requires a rollback guard."
                )

            try:
                rollback_state = (
                    self._rollback_runtime_binding_guarded_for_identity(
                        working,
                        rollback_guard,
                        restore_binding,
                    )
                ...
                # Rollback may have lost to a newer writer.
                # Inspect newest Forest truth before deciding
                # which runtime sessions are safe to end.
                try:
                    latest = self.load_state()
                    ...
                    # Fail closed. If durable canonical truth
                    # cannot be established, end nothing.
                    canonical_known = False
                ...
                # Certified C1 semantics guarantee that a
                # successful rollback restored this exact old
                # target binding or removed the new-only slot.
                canonical_known = True
                canonical_session_id = (
                    restore_session_id
                )
            ...
            # Detached/in-memory handoff never advanced
            # durable Forest truth. The old target remains
            # the only rollback anchor worth preserving.
            rollback_state = working
            canonical_known = True
            canonical_session_id = (
                restore_session_id
            )
        ...
        if canonical_known:
            for session_id in candidate_session_ids:
                if (
                    restore_session_id is not None
                    and session_id
                    == restore_session_id
                ):
                    continue
    ...
    def _retire_successful_model_form_handoff_sessions(
        self,
        *,
        runtime_adapter,
        execution_state,
        ...
        """Retire obsolete handoff sessions after E4D success.

        This helper performs runtime cleanup only. It must be
        called only after Forest semantic commit succeeds.
        The final canonical runtime session is never retired.
        """
        ...
        if not isinstance(
            ...
            raise TaskSessionError(
                "Successful handoff retirement requires "
                "a session-ID collection."
            )
        ...
        if final_session_id is None:
            raise TaskSessionError(
                "Successful handoff retirement requires "
                "the final canonical session ID."
            )
        ...
        if not final_session_id:
            raise TaskSessionError(
                "Final canonical handoff session ID "
                "cannot be empty."
            )
    ...
    def _send_runtime_turn_for_frozen_model_form(
        ...
        *,
        ...
        # ----------------------------------------------
        # MODEL FORM AUTO ELIGIBILITY:
        # determine once before any Auto router work.
        # ----------------------------------------------
        try:
            auto_control = (
                model_form_control_from_mapping(
            ...
            auto_task = self._active_runtime_task(
                control_state
            )

            automatic_model_form_eligible = (
                automatic_model_form_is_eligible(
                    auto_control,
            ...
            raise TaskSessionError(
                ...
                "is eligible for this turn."
            ) from exc

        # ----------------------------------------------
        # AUTOMATIC MODEL FORM ROUTER:
        # run exactly once only when Auto is eligible
        # and no legacy automatic form was supplied.
        # ----------------------------------------------
        automatic_model_form_recommendation = None

        if (
            automatic_model_form_eligible
            and automatic_model_form is None
        ):
            try:
                ...
                raise TaskSessionError(
                    "Forest automatic Model Form "
                    "routing backend is unavailable."
                ) from exc

            try:
                automatic_model_form_routing_input = (
                    AutomaticModelFormRoutingInput(
                        task_id=auto_task["id"],
                        execution_context_id=(
                            execution_context_id
                ...
                automatic_model_form_recommendation = (
                    recommend_automatic_model_form(
                        automatic_model_form_routing_input
                    )
                )
                ...
                raise TaskSessionError(
                    ...
                    "Model Form for this turn."
                ) from exc

        turn_automatic_model_form = (
            automatic_model_form
        )
        turn_automatic_model_form_reasons = ()
        ...
        if (
            ...
            if not isinstance(
                ...
                raise TaskSessionError(
                    "Automatic Model Form router must return "
                    "AutomaticModelFormRecommendation."
                )

            if (
                automatic_model_form_recommendation.task_id
                != automatic_model_form_routing_input.task_id
            ):
                raise TaskSessionError(
                    "Automatic Model Form recommendation "
                    "does not belong to the active Task."
                )
            ...
            turn_automatic_model_form = (
                automatic_model_form_recommendation.form
            )

            turn_automatic_model_form_reasons = (
                automatic_model_form_recommendation.reasons
            )
        ...
        # ----------------------------------------------
        # RESOURCE GOVERNOR: optional pre-runtime gate.
        # ----------------------------------------------
        #
        # The request is built only after Reasoning and Model
        # Form have been frozen for this meaningful turn.
        #
        # Approval continues into the already-certified E4E5
        # transaction unchanged. Defer / deny stop here, before
        # handoff preparation or any runtime/session side effect.
        resource_governor_decision = None
        ...
        if resource_governor is not None:
            if not callable(
                ...
                raise TaskSessionError(
                    "resource_governor must be callable "
                    "or None."
                )
            ...
            try:
                ...
                raise TaskSessionError(
                    "Forest resource-governance backend "
                    "is unavailable."
                ) from exc
            ...
            try:
                ...
                raise TaskSessionError(
                    "Could not prepare Forest "
                    "ResourceRequest for this turn."
                ) from exc
            ...
            try:
                ...
                raise TaskSessionError(
                    "Forest resource Governor could "
                    "not evaluate this turn."
                ) from exc
            ...
            if not isinstance(
                ...
                raise TaskSessionError(
                    "Resource Governor must return "
                    "GovernorDecision."
                )

            if (
                resource_governor_decision.request
                is not resource_request
            ):
                raise TaskSessionError(
                    "Resource Governor decision does "
                    "not belong to the exact "
                    "ResourceRequest."
                )

            governor_outcome = (
                resource_governor_decision.outcome
            )

            if governor_outcome == "defer":
                error = TaskSessionError(
                    "Forest resource Governor "
                    "deferred this turn."
                )
                error.resource_governor_decision = (
                    resource_governor_decision
                )
                raise error

            if governor_outcome == "deny":
                error = TaskSessionError(
                    "Forest resource Governor "
                    "denied this turn."
                )
            ...
            if governor_outcome != "approve":
                raise TaskSessionError(
                    "Resource Governor returned an "
                    "unsupported outcome."
                )

        elif (
            resource_priority is not None
            or resource_request_source is not None
            or resource_request_reasons != ()
        ):
            raise TaskSessionError(
                "Resource governance metadata requires "
                "resource_governor."
            )
        ...
        # ----------------------------------------------
        # MODEL FORM HANDOFF TRANSACTION
        # ----------------------------------------------
        # Runtime sessions created during an actual
        # handoff remain transaction-owned until E4D
        # establishes semantic Forest truth.
        handoff_transaction_session_ids = []
        handoff_rollback_guard = None

        working = initial
        execution_state = initial_execution_state

        if model_form_handoff_required:
            initial_handoff_session_id = str(
                handoff_bootstrap_session_id
                or ""
            ).strip()

            if not initial_handoff_session_id:
                raise TaskSessionError(
                    "Actual Model Form handoff "
                    "produced no initial transaction "
                    "session ID."
                )

            handoff_transaction_session_ids.append(
        ...
        try:
            ...
            if (
                ...
                try:
                    ...
                    if ensured["created"]:
                        if not model_form_handoff_required:
                            self._best_effort_end_runtime_sessions(
                                runtime_adapter,
                                [requested_session_id],
                                execution_state,
                ...
                if model_form_handoff_required:
                    handoff_rollback_guard = copy.deepcopy(
                        persist_guard
                    )
            ...
            try:
                ...
                if model_form_handoff_required:
                    ...
                    try:
                        ...
                        recovery_handoff_session_id = str(
                            recovery_handoff_bootstrap.get(
                                "session_id"
                            )
                            or ""
                        ).strip()

                        if not recovery_handoff_session_id:
                            raise TaskSessionError(
                                "Continuity-backed stale handoff "
                                "replacement returned no session ID."
                            )

                        if (
                            recovery_handoff_session_id
                            not in handoff_transaction_session_ids
                        ):
                            handoff_transaction_session_ids.append(
                                recovery_handoff_session_id
                            )
            ...
            if model_form_handoff_required:
                try:
                    self._cleanup_failed_model_form_handoff_transaction(
                        working=working,
                        frozen_turn=frozen_turn,
                        runtime_adapter=
                            runtime_adapter,
                        ...
                        # A durable C1 rollback is necessary
                        # only after this handoff actually
                        # advanced Forest's runtime binding.
                        persist=bool(
                            persist
                            and handoff_rollback_guard
                            is not None
                        ),
                    ...
                    # failure. B2A itself fails closed when
                    # canonical runtime ownership is unknown.
                    pass
        ...
        # E4D has now established semantic Forest truth.
        # Only at this point may the old target and
        # obsolete handoff-created sessions retire.
        if model_form_handoff_required:
            try:
                self._retire_successful_model_form_handoff_sessions(
                    runtime_adapter=
                        runtime_adapter,
                    execution_state=
                        execution_state,
                ...
                # Retirement is best-effort cleanup.
                # Semantic success must not be converted
                # into a failed user turn after E4D commits.
                pass
    ...
    def send_runtime_turn(
        ...
        if execution_context_id is not None:
            return (
                self._send_runtime_turn_for_frozen_model_form(
                    ...
                    automatic_model_form=
                        automatic_model_form,
                        automatic_model_form_evidence=automatic_model_form_evidence,
                    resource_governor=
                        resource_governor,
                    resource_priority=
                        resource_priority,
        ...
        if (
            resource_governor is not None
            or resource_priority is not None
            or resource_request_source is not None
            or resource_request_reasons != ()
        ):
            raise TaskSessionError(
                "Resource governance requires an "
                "explicit execution_context_id."
            )
