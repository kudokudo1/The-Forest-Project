...
def main():
    ...
    for name in active_ids:
        ...
        if kind == "skill":
            ...
            if mapping:
                runtime_skill = mapping["target"]
                hermes_skills.append(runtime_skill)
                skill_bindings.append(
                    {
                        "canonical_id": name,
                        "runtime_id": runtime_skill,
                        "adapter": "hermes",
                    }
                )
    ...
    try:
        ...
        if not isinstance(task_session, dict):
            task_session = {}
            state["task_session"] = task_session

        task_session["task_sticky_skills"] = skill_bindings
