---
title: Project Forest, Project Digital Cross, and Project Digital Fortress — Expanded Continuity Packet
aliases:
...
# Project Forest, Project Digital Cross, and Project Digital Fortress — Expanded Continuity Packet
...
# 0. Latest Delta — 2026-08-07

> [!important]
> This section records the newest state that should override older conflicting snapshots elsewhere in this packet.

## Current Bristlecone / Hermes / Newelle priority

The current priority is to make **Bristlecone Pine, Hermes, and the optional Newelle path as fast and reliable as practical without weakening Qubes isolation or removing important Treewright capabilities**.

Current retained direction:

- **Hermes remains Bristlecone's primary orchestration layer.**
- **Ollama remains the local model runtime.**
- **Newelle may continue to be tested and optimized, but it is not yet trusted as the daily critical path.**
- After a recent nap/restart/reset, the user reported a **connection error**. This is a current unresolved troubleshooting item and should not be assumed fixed.
- Speed work should preserve a known-good baseline and benchmark changes rather than making many changes at once.

Latest confirmed Hermes prompt/tool reductions:
...
Tools:         ~14
Later interactive count: ~15 with Cron intentionally retained
...
The API server reduction was approximately:
...
Hermes cold first-turn behavior has been dramatically slower than direct Ollama, often on the order of minutes. The working diagnosis remains **cold prompt/context prefill and agent/tool overhead**, not basic model load time.

Additional trimmed Hermes benchmark order retained from testing:
...
Stripped Newelle tests retained:

```text
Run 1: output after 5m 25.65s
Run 2: stopped after ~15m with no answer — failed
Run 3: Newelle welcome/suggestion prompts after 5m 42.23s, but no Bristlecone answer — failed
```

This means Newelle remains useful as an experimental Bark/interface candidate, but Hermes is still the stronger foundation for Forest learning, multi-model work, tools, and long-term orchestration.

## Current storage/hardware delta relevant to Qubes

The user reports approximately **2.5 TB of installed PC storage** in the current machine arrangement:

- One **1 TB SSD** contains/preserves Windows and should remain untouched for dual boot.
- A second **1 TB SSD** is mostly games and is the main candidate for eventual Qubes repurposing.
- A **500 GB HDD** currently appears associated with the Qubes installation, but reported free/used space does not make sense to the user: the system appears roughly half-full while only about **10 GB remaining** is being shown somewhere.
- Do not assume Qubes itself occupies ~480 GB. The reported 10 GB may refer to a specific filesystem/root volume rather than the entire physical disk or LVM thin pool.
- Before any destructive storage operation, audit the physical disks, partitions, dom0 root usage, and Qubes thin-pool usage.
- The second 1 TB Windows/game SSD should be inspected and mounted **read-only first** before repurposing.
- Game saves, mods, screenshots, and any non-cloud data should be preserved before formatting that SSD.

Spare hardware currently available:

- **NVIDIA Quadro K4200, 4 GB VRAM** — older spare GPU.
- An additional HDD is available, but the user currently does not have a SATA data cable for it.
- The K4200 is not a preferred current Ollama accelerator; it may be more useful as experimental passthrough hardware or later NAS/media hardware depending on compatibility.
- The K4200 does **not** need a SATA data cable; the spare HDD does.

## Current thermal snapshot under real workload

While running Qubes with three browsers, multiple tabs, and ChatGPT, observed sensors were approximately:
...
jc42 memory:  108–112 F / ~42–44 C
/dev/sda:      91 F / ~33 C
/dev/sdb:      32 F / ~0 C  (likely invalid/missing sensor reading)
```

These temperatures were not considered alarming for the active workload. Storage pressure is currently a more important operational concern than the observed thermals.
...
> [!important]
> **Qubes information is intentionally separated here from the Forest/PDC/PDF conceptual material above.**
> For full machine details, resource settings, hardware, temperatures, storage, service diagnostics, and performance troubleshooting, use the companion file:
> `Qubes_PC_System_Expanded_Technical_Context_2026-08-07.md`
